#!/usr/bin/env bash
# versiona.sh - Release automation for terust.
#
# Flow:
...
printf '\n%b\n' "${BOLD}=== terust release ${VERSION} ===${RESET}"
...
git tag -a "$TAG_VERSION" -m "terust ${VERSION}" -m "$CHANGELOG"
...
gh release create "$TAG_VERSION" \
    --title "terust ${VERSION}" \
    --notes-file "$NOTES_FILE"

ok "terust ${VERSION} released as ${TAG_VERSION}."
